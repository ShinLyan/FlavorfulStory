...
//Copyright prortected under Unity Asset Store EULA
...
Varyings LitPassVertex(Attributes input)
{
	...
	// Упростить, если tangentToWorld не нужен:
...
void ModifySurfaceData(Varyings input, out SurfaceData surfaceData)
{
	...
	if (_HeightGradientColor.a > 0.001)
	{
		float heightMask = saturate((input.positionWS.y - _HeightGradientMin) / (_HeightGradientMax - _HeightGradientMin));
		albedoAlpha.rgb = lerp(albedoAlpha.rgb, _HeightGradientColor.rgb, heightMask * _HeightGradientColor.a);
	}
