...
namespace FlavorfulStory.QuestSystem.Objectives.Params
{
    ...
    public abstract class ObjectiveParamsBase
    {
        ...
        public void CheckAndComplete(QuestStatus status, QuestExecutionContext context, object eventData = null)
        {
        ...
        protected abstract bool ShouldComplete(QuestExecutionContext context, object eventData);

        /// <summary> Завершает цель, если она найдена в текущем квесте. </summary>
        /// <param name="status"> Статус квеста. </param>
        /// <param name="context"> Контекст выполнения цели. </param>
        private void CompleteObjective(QuestStatus status, QuestExecutionContext context)
        {
