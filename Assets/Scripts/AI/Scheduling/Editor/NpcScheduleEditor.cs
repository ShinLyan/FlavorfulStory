...
public class NpcScheduleEditor : Editor
{
    ...
    /// <summary> Обрабатывает изменение режима PlayMode для обновления списка локаций. </summary>
    ...
    /// <param name="demonstrator"> Ссылка на NpcScheduleDemonstrator. </param>
    /// <returns> Скорректированная позиция точки. </returns>
    private Vector3 GetSurfaceAdjustedPosition(Vector3 position, NpcScheduleDemonstrator demonstrator)
    {
        var ray = new Ray(position + Vector3.up * 50f, Vector3.down);
        if (Physics.Raycast(ray, out var hit, 100f, demonstrator.GroundMask))
            return new Vector3(
                position.x,
                hit.point.y + demonstrator.GroundHeight,
                position.z
            );
        return position;
    }
    ...
    private void HandleParameterCreation(NpcScheduleDemonstrator demonstrator)
    {
        EditorGUILayout.BeginHorizontal();
        {
                                               ...
                                               demonstrator.Schedule.Params.Length == 0))
            {
                if (GUILayout.Button("Remove Last Parameter"))
                {
                    ...
                    if (demonstrator.Schedule.Params.Length > 0)
                    {
    ...
    private void HandleParameterSelection(NpcScheduleDemonstrator demonstrator)
    {
        EditorGUI.BeginChangeCheck();
    ...
    private void ShowPathManagement(NpcScheduleDemonstrator demonstrator)
    {
        var currentParam = demonstrator.Schedule.Params[demonstrator.SelectedParamIndex];
    ...
    private void AddSchedulePoint(NpcScheduleDemonstrator demonstrator)
    {
        var currentParam = demonstrator.Schedule.Params[demonstrator.SelectedParamIndex];
    ...
    private void DeleteLastSchedulePoint(NpcScheduleDemonstrator demonstrator)
    {
        var currentParam = demonstrator.Schedule.Params[demonstrator.SelectedParamIndex];
        var newPath = new SchedulePoint[currentParam.Path.Length - 1];
