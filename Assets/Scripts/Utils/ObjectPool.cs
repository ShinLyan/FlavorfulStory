...
namespace FlavorfulStory.Utils
{
  /// <summary> Обобщённый пул объектов. </summary>
    public class ObjectPool<T> : IDisposable
    {
      ...
      /// <param name="initFunc">Функция инициализации объекта (необязательная).</param>
      /// <param name="deInitFunc">Функция деинициализации объекта (необязательная).</param>
      /// <param name="actionOnRelease">Действие при возврате объекта (необязательное).</param>
      /// <param name="actionOnDestroy">Действие при уничтожении объекта (необязательное).</param>
      ...
      /// <summary> Поолучить объект из пула или создать новый, если пул пуст. </summary>
      public T Get()
      {
        T obj;
        if (_list.Count == 0)
